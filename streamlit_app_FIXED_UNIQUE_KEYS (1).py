                    ...
                    with chart_placeholder.container():
                        ...
                        with c1:
                            st.plotly_chart(
                                ...
                                use_container_width=True,
                                key=f"live_sentiment_{run_id}_{render_seq}",
                            )
                        with c2:
                            st.plotly_chart(
                                make_rating_donut(current_df),
                                use_container_width=True,
                                key=f"live_rating_{run_id}_{render_seq}",
                            )
...
if isinstance(results_df, pd.DataFrame) and not results_df.empty:
    ...
    with tab1:
        ...
        with a:
            st.plotly_chart(
                ...
                use_container_width=True,
                key=f"final_sentiment_{run_summary['run_id']}",
            )
        with b:
            st.plotly_chart(
                make_rating_donut(results_df),
                use_container_width=True,
                key=f"final_rating_{run_summary['run_id']}",
            )
    ...
    with tab2:
        st.plotly_chart(
            ...
            use_container_width=True,
            key=f"final_confidence_{run_summary['run_id']}",
        )
        ...
        st.plotly_chart(
            heatmap,
            use_container_width=True,
            key=f"final_heatmap_{run_summary['run_id']}",
        )
